# dataset config
_base_ = [
    "../_base_/datasets/allo_1080x1080.py",
...
model = dict(
    backbone=dict(
        img_size=1080,
        init_cfg=dict(
            checkpoint="checkpoints/dinov2_converted_1080x1080_p15.pth",
        ),
        patch_size=15,
        reins_config=dict(
            patch_size=15
    ...
    data_preprocessor=dict(
        mean=[53.1165, 54.1620, 54.7230],
        std=[68.0595, 68.8500, 69.1305],
        size=crop_size,
    ),
    test_cfg=dict(
        crop_size=crop_size,
...
default_hooks = dict(
    ...
    checkpoint=dict(
        type="CheckpointHook", by_epoch=False, interval=10000, max_keep_ckpts=2
    ),
