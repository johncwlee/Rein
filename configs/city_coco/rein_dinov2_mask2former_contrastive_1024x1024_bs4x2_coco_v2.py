...
train_pipeline = [
    ...
    dict(type="MixCOCOV2", 
         coco_root="/home/johnl/data/coco/",    #* Change this to your own path
         mix_pipeline=[
            dict(
                type="RandomChoiceResizeV2",
                scale_factors=[0.1 * x for x in range(2, 11)]+[0.5, 0.5, 0.75, 0.75],
                interpolation="area"
            ),
            dict(type="CenterCrop", crop_size=crop_size),
            dict(type="RandomFlip", prob=0.5)],   
         ood_pipeline=[
            dict(type="CenterCrop", crop_size=crop_size, auto_pad=True)]), 
    dict(type="PhotoMetricDistortion"),
    dict(type="PackSegInputsWithCOCO"),
]
