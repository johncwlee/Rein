# dataset config
_base_ = [
    "../_base_/datasets/cityscapes_vistas_1024x1024.py",
...
model = dict(
    backbone=dict(
        img_size=1024,
        init_cfg=dict(
            checkpoint="checkpoints/dinov2_1024x1024_p16_rein_city+vistas_combined.pth",
            prefix="backbone.",
        ),
    ),
    decode_head=dict(
...
train_pipeline = [
    ...
    dict(type="MixADE", root="/home/johnl/data/ADEChallengeData2016"), #* Change this to your own path
    dict(
...
train_cityscapes = dict(
    ...
    data_root={{_base_.cityscapes_root}},
    data_prefix=dict(
...
train_cfg = dict(type="IterBasedTrainLoop", max_iters=2000, val_interval=100000)
...
default_hooks = dict(
    ...
    checkpoint=dict(
        type="CheckpointHook", by_epoch=False, interval=1000, max_keep_ckpts=3
    ),
